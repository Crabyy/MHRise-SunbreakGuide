...
- Reworked the **Home** welcome copy from the generic "Welcome to Monster Hunter Rise: Sunbreak Utility." to a warmer, more personal line about why the app exists. Tried a hero image + two-column layout and a set of quick-launch cards to the three main sections first, per Craby's ask to improve the landing page, but both were rejected on review (card links felt like unnecessary utility clutter; the hero image "didn't fit" the look) — reverted to the original single-column layout with just the new copy.
- Fixed a real bug found while touching that section: `.home-disclaimer` is a `<small>` element, which is `display: inline` by default, so its `margin-top: 24px` was silently doing nothing — the Capcom disclaimer line sat almost flush against "Made by Craby" above it. Added `display: block` so the intended spacing actually applies.
- Simplified **Monster Matchups** search to monster names only, per Craby's request. It previously also matched against the recommended weapon, element, rampage decoration, and (LBG) ammo/type text, so a query like "Elembane" would surface every monster recommending that rampage skill — surprising when the placeholder just said "Search monster, weapon, ammo…" and the box sits right next to a dedicated element filter. Placeholder is now "Search monster…".
